...
export const getAllVehicles = async (req: Request, res: Response) => {
  try {
    ...
    if (result.rows.length === 0) return sendResponse(res, 200, 'No vehicles found', []);
...
export const getVehicleById = async (req: Request, res: Response) => {
  try {
    ...
    if (result.rows.length === 0) return sendError(res, 404, 'Vehicle not found');
...
export const updateVehicle = async (req: Request, res: Response) => {
  ...
  try {
    const result = await pool.query(
      `UPDATE vehicles SET ${fields}, updated_at = NOW() WHERE id = $1 RETURNING *`,
      [vehicleId, ...values]
    );
    if (result.rows.length === 0) return sendError(res, 404, 'Vehicle not found');
